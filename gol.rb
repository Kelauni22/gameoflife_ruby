...
board[2][2], board[2][3], board[2][4], board[3][3], board[3][4] = 1,1,1,1,1
...
while i < 3 #make this a while true later so that it runs infinitely
  ...
  board.each_with_index do |x, x_index| #get row index number and value
    ...
    #check number of alive/dead cells around this particular cell
      ...
      n = 0
      for cell in cells_array #this should return n
        if cell == 1
          n+=1
        end
      end
      if y == 1 #check to see if this particular cell is alive
        #If <= 1 cells around target are 1 or if > 3 cells around target are 1 , make target 0
        #If 2 or 3 cells around target are 1, don't change target
        y = 0 if n <= 1 || n > 3
      end
      if y == 0
